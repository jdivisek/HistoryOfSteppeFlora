...
chorotype <- clas.spe$chorotype
names(chorotype) <- clas.spe$species

##species classification into ecological groups
ecotype <- rep(NA, nrow(clas.spe))
names(ecotype) <- clas.spe$species
ecotype[clas.spe$species[!is.na(clas.spe$meadow.steppe)]] <- 1 #"Meadow steppe"
ecotype[clas.spe$species[!is.na(clas.spe$grass.steppe)]] <- 2 #"Grass steppe"
ecotype[clas.spe$species[!is.na(clas.spe$rocky.steppe)]] <- 3 #"Rocky steppe"

##paths to models
mod.paths <- paste0(getwd(),"/Source areas LGM/", clas.spe$species, ".tif")
...
set.seed(123)
test.chorotype <- adonis2(cgrs ~ chorotype, method="bray", sqrt.dist=T, by = NULL, binary=TRUE, permutations=9999)
set.seed(123)
test.ecotype <- adonis2(cgrs[names(ecotype)[!is.na(ecotype)],] ~ ecotype, method="bray", sqrt.dist=T, by = NULL, binary=TRUE, permutations=9999)
...
#Permutational Multivariate Analysis of Variance Using Distance Matrices
set.seed(123)
test.chorotype.MH <- adonis2(cgrs ~ chorotype, method="bray", sqrt.dist=T, by = NULL, binary=TRUE, permutations=9999)
set.seed(123)
test.ecotype.MH <- adonis2(cgrs[names(ecotype)[!is.na(ecotype)],] ~ ecotype, method="bray", sqrt.dist=T, by = NULL, binary=TRUE, permutations=9999)
