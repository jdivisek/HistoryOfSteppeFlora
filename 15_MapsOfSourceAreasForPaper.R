###############################################################################
###                      MAPS OF LGM SOURCE AREA FOR PAPER                  ###
###############################################################################
...
colnames(cgrs.range) == clas.spe$abbrev
colnames(cgrs.range) <- clas.spe$species
...
mod <- projectRaster(from=mod, to=ice, method = "ngb")
# plot(mod)
...
mod2 <- projectRaster(from=mod2, to=ice, method = "ngb")
# plot(mod2)
...
#plot
tiff("Figure_3.tif", width = 9.8, height = 5.5, units = "in", res=500, compression = "lzw")
layout(matr)
# layout.show(20)
...
for(i in 1:nlayers(mod))
{
  ...
  if(i == 1) {rs <- rs[[which(gsub("_", " ", names(rs)) %in% clas.spe$species[!is.na(clas.spe$rocky.steppe)])]]}
  if(i == 2) {rs <- rs[[which(gsub("_", " ", names(rs)) %in% clas.spe$species[!is.na(clas.spe$grass.steppe)])]]}
  if(i == 3) {rs <- rs[[which(gsub("_", " ", names(rs)) %in% clas.spe$species[!is.na(clas.spe$meadow.steppe)])]]}

  maxValue(rs)
  rs <- rs[[which(maxValue(rs) == 1)]]
  n <- nlayers(rs)
  text(-11, 69.5, paste0("N = ", n), pos=4)
  ...
  axis(2, at=c(40,60), labels = c("40°N", "60°N"))
  if(i == 3) {axis(1, at=c(0,20,40,60), labels = c("0°", "20°E", "40°E", "60°E"))}
  ...
  if(i == 1) {div <- rowSums(cgrs.range[, !is.na(clas.spe$rocky.steppe)])}
  if(i == 2) {div <- rowSums(cgrs.range[, !is.na(clas.spe$grass.steppe)])}
  if(i == 3) {div <- rowSums(cgrs.range[, !is.na(clas.spe$meadow.steppe)])}
  
  cgrs.wgs84$div <- div[cgrs.wgs84$CGRSNAME]
  cgrs.wgs84$div[is.na(cgrs.wgs84$div)] <- 0
  max(div)
  ...
  cgrs.wgs84$div.clas.spe <- cut(cgrs.wgs84$div, breaks=brks, labels=seq(1, length(brks)-1), include.lowest = FALSE, right=TRUE) 
  
  plot(cgrs.wgs84, add=T, col=cols[cgrs.wgs84$div.clas.spe], border=NA)
  
  plot(m, add=T, lwd=0.4)
  box()
  
  ##add no of species
  if(i ==1) {n <- sum(!is.na(clas.spe$rocky.steppe))}
  if(i ==2) {n <- sum(!is.na(clas.spe$grass.steppe))}
  if(i ==3) {n <- sum(!is.na(clas.spe$meadow.steppe))}
...
##Source areas in the LGM
mod <- stack(paste0("./Stacked source areas LGM/Chorotype/Chorotype ", 1:5, ".sdat"))
mod <- projectRaster(from=mod, to=ice, method = "ngb")
# plot(mod)

##Source areas in the mid-Holocene
mod2 <- stack(paste0("./Stacked source areas mid-Holocene/Chorotype/Chorotype ", 1:5, ".sdat"))
mod2 <- projectRaster(from=mod2, to=ice, method = "ngb")
# plot(mod2)

maxValue(mod)
maxValue(mod2)
max.brk <- c(20, 20, 24, 22, 4)

#plot
tiff("Figure_4.tif", width = 9.8, height = 9, units = "in", res=500, compression = "lzw")
layout(matr)
# layout.show(20)
par(mar=c(0.4,0.4,0.4,0.4), oma=c(2,5,4,0.5), tcl=-0.3, mgp=c(3, 0.5, 0))
...
for(i in 1:nlayers(mod))
{
  ...
  #add no of species
  rs <- stack(list.files("./Source areas LGM", full.names = T))
  rs <- rs[[which(gsub("_", " ", names(rs)) %in% clas.spe$species[clas.spe$chorotype == i])]]
  maxValue(rs)
  rs <- rs[[which(maxValue(rs) == 1)]]
  n <- nlayers(rs)
  text(-11, 69.5, paste0("N = ", n), pos=4)
  ...
  axis(2, at=c(40,60), labels = c("40°N", "60°N"))
  if(i == 5) {axis(1, at=c(0,20,40,60), labels = c("0°", "20°E", "40°E", "60°E"))}
  ...
  if(i == 1) {div <- rowSums(cgrs.range[, clas.spe$chorotype == 1])}
  if(i == 2) {div <- rowSums(cgrs.range[, clas.spe$chorotype == 2])}
  if(i == 3) {div <- rowSums(cgrs.range[, clas.spe$chorotype == 3])}
  if(i == 4) {div <- rowSums(cgrs.range[, clas.spe$chorotype == 4])}
  if(i == 5) {div <- rowSums(cgrs.range[, clas.spe$chorotype == 5])}
