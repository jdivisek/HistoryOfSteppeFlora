...
migrate <- function(data, lgm.land, rates, temp.folder, out.folder, grid.path, saga.env, glm.env.data, cgrs.range, country)
{
  ...
  refs <- getRef(rs[[1]], threshold = data[[1]], min.size = 42) #min size in No cells in one spetial cluster. 416.493 corresponds to 10 000 km2 with a raster resolution of 4900 m
  ...
  if(!is.null(refs))
  {
    ...
    {
      ###claculate accessibility fro each potential refugia
      ...
      ###RUN GLM MODELS
      acces.test[[w]] <- glm.test2(data=glm.data, y="species", keep.in=colnames(glm.env.data), 
    ...
    if(any(sum.imp > 0))
    {
      ...
      if(sum(c(test$D.diff > 0 & test$Estimate > 0 & test$`Pr(>|z|)` < 0.05), na.rm = T) > 0)
      {
        ...
        plot(eref, axes=F, box=F, legend=T, main=paste0("Effective LGM refugia for ", sn))
        plot(country, add=T, lwd=0.3)
      }
      else
      {
