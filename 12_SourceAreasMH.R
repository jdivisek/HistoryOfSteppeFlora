...
for(q in names(me))
{
  ...
  refs <- getRef(rs[[1]], threshold = th, min.size = 42) #42 cells ~ 1000 km2
  # plot(refs)
  ...
  if(!is.null(refs) & !is.na(best.mig.rate[q]))
  {
    ...
    refs <- getRef(rs[[7]], threshold = th, min.size = 42)
  ...
  ###TEST SOURCE AREAS IN MID-HOLOCENE
  if(is.null(refs) | is.na(best.mig.rate[q])){
    acces.test.MH[[q]] <- NA}
  if(!is.null(refs) & !is.na(best.mig.rate[q]))
  {
    #save MH potential accesible refugia
    ...
    ###RUN GLM MODELS
    acces.test.MH[[q]] <- glm.test(data=glm.data, y="species", keep.in=colnames(glm.env.data), 
